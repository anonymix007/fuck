...
static void setup_leak()
{
  if (!write_file(KMEMLEAK_FILE, "scan")){
    perror("Failed to write `scan` to KMEMLEAK_FILE\n");
    exit(1);
  }
  sleep(5);
  if (!write_file(KMEMLEAK_FILE, "scan")){
  ...
  if (!write_file(KMEMLEAK_FILE, "clear")){
    perror("Failed to write `clear` to KMEMLEAK_FILE\n");
    exit(1);
  }
}
...
{
  intptr_t res = 0;
  NONFAILING(memcpy((void*)0x20000180, "/dev/fd#\000", 9));
  res = -1;
  NONFAILING(res = syz_open_dev(0x20000180, 0, 0x800));
  if (res != -1)
    r[0] = res;
  NONFAILING(memcpy((void*)0x20000180, "/dev/fd#\000", 9));
  res = -1;
  NONFAILING(res = syz_open_dev(0x20000180, 3, 0x200));
  if (res != -1)
    r[1] = res;
  syscall(__NR_ioctl, r[1], 0x24b, 0);
  NONFAILING(*(uint32_t*)0x20000000 = 0x132);
  NONFAILING(*(uint32_t*)0x20000004 = 1);
  NONFAILING(*(uint32_t*)0x20000008 = 0xac1);
  NONFAILING(*(uint32_t*)0x2000000c = 4);
  NONFAILING(*(uint32_t*)0x20000010 = 6);
  syscall(__NR_ioctl, r[1], 0x4014024c, 0x20000000ul);
